...
export default function ParkingReservationTabs() {
  ...
  const fetchReservations = async () => {
    setLoading(true);
    try {
      const data = await parkingAPI.getReservations();
  ...
  const renderModals = () => (
    <>
      ...
      {isVehicleModalOpen && selectedVehicleForModal && (
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="aspect-video relative rounded-3xl overflow-hidden bg-slate-100 shadow-inner">
                    <img
                      src={getPhotoUrl(selectedVehicleForModal.photos) || '/placeholder-car.jpg'}
                      alt=""
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    {getAllPhotoUrls(selectedVehicleForModal.photos).slice(0, 4).map((p, i) => (
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedVehicleForModal.garantie && (
                      <span className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                        <FontAwesomeIcon icon={faShieldAlt} /> Garantie
                      </span>
                    )}
                    {selectedVehicleForModal.assurance && (
                      <span className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                        <FontAwesomeIcon icon={faCheckCircle} /> Assurance
                      </span>
                    )}
                    {selectedVehicleForModal.chauffeur && (
                      <span className="px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                        <FontAwesomeIcon icon={faUserTie} /> Chauffeur
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="bg-slate-50 rounded-xl sm:rounded-3xl p-4 sm:p-6 md:p-8 border border-slate-100">
              ...
              <div className="flex flex-wrap gap-3 sm:gap-4">
                ...
                {/* Commission button removed for parking */}
              </div>
            </div>
          </div>
        </div>
                ...
                paginatedReservations.map((res) => (
                  <tr key={res.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td 
                      ...
                      onClick={() => {
                        setSelectedVehicleForModal(res.vehicle);
                    ...
                    <div className="space-y-2 sm:space-y-3 mb-3 flex-1">
                      ...
                      <div className="flex justify-between items-center bg-orange-50/50 p-2 rounded-lg border border-orange-100">
                        ...
                        <span className="font-black text-orange-600 text-[10px] sm:text-xs truncate max-w-[100px] sm:max-w-[120px]">
                          {res.vehicle?.plate || res.vehicle?.immatriculation || '-'}
                        </span>
                      </div>
                      ...
                      <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
                        <div className="flex gap-1">
                          ...
                          {res.status !== 'COMPLETED' && res.status !== 'CANCELED' && (
                            ...
                            >
                              <FontAwesomeIcon icon={res.status === 'PENDING' ? faTimes : faBan} size="sm" />
                            </button>
                          )}
                        </div>
